...
Stay synchronized with CORS state changes:

```typescript
// Listen for status changes
const handleStatusChange = (status) => {
  console.log('CORS status updated:', status)
  // status: { enabled: boolean, credentials: boolean }
}

appCors.onChange.addListener(handleStatusChange)

// Clean up when done
appCors.onChange.removeListener(handleStatusChange)
```
...
function CORSManager() {
  ...
  useEffect(() => {
    ...
    // Monitor CORS changes
    const handleChange = (status) => setCorsStatus(status)
    appCors.onChange.addListener(handleChange)
    
    return () => appCors.onChange.removeListener(handleChange)
  }, [])
